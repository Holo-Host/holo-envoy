...
describe("Resiliency", () => {
  ...
  it('Should recover from a browser tab closed during zome call once successfully signed back in', async function () {
  ...
  it.only('Should recover from internet loss (client closing) in the middle of a zome call', async function() {
    ...
    browserClient.on('Network.webSocketFrameSent', async ({ response }) => {
      if (JSON.parse(response.payloadData).method === 'holo/call') {
        ...
        console.log('envoy ws server.clients : ', server.clients)
        await setup.close_connections(server.clients)
      }
    })
    ...
    // chaperone should receive a connection timeout error as client is down
    ...
    const { hasSignedIn, responseSuccess } = await page.evaluate(async function (host_agent_id, registered_happ_hash, SIGNED_IN_AGENT_HASH) {
    ...
    }, HOST_AGENT_ID, REGISTERED_HAPP_HASH, SIGNED_IN_AGENT_HASH)
