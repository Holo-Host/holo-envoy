...
describe("Resiliency", () => {
  ...
  it("should recover from closed browser tab", async function() {
    ...
    await page.exposeFunction('checkBrowserState', () => {
      console.log(' ---------------------------------> browser_handler.browser exists ?, browser_handler.browser obj : ',!!browser_handler.browser, browser_handler.browser)
      return !!browser_handler.browser
    })
    ...
    const { hasSignedUp, responsefailure, responseSuccess } = await page.evaluate(async function (host_agent_id, registered_happ_hash, joiningCode, SIGNED_IN_AGENT_HASH) {
      ...
      let browserExists = window.checkBrowserState()
      // let intervalId, timeoutId;
      // while (browserExists !== false) {
      //   console.log('...waiting for browser to be disconnected.')
      //   intervalId = setInterval(console.log('Browser State: ', browserExists), 1000)
      //   timeoutId = setTimeout(() => {
      //     clearInterval(intervalId)
      //     clearTimeout(timeoutId)
      //     throw new Error('...browser failed to disconnect')
      //   }, 9000)
      // }
      // clearInterval(intervalId)
      // clearTimeout(timeoutId)

      console.log(' ----------------->>>>>>> browserExists : ', browserExists)
      expectEquals(browserExists, false)
      ...
      if (browserExists === false) {
        ...
        throw new Error('did not disconnect browser')
      }
  ...
  it.only("should recover from host shutting off in middle of zome call", async function() {
    ...
    // set the error to a scenario var against which we can test
    ...
    const { hasSignedUp, responsefailure } = await page.evaluate(async function (host_agent_id, registered_happ_hash, joiningCode, SIGNED_IN_AGENT_HASH) {
      let hasSignedUp = false
      console.log("Registered Happ Hash: %s", registered_happ_hash);
      const client = new Chaperone({
        "mode": Chaperone.DEVELOP,
