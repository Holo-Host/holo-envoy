...
const EC_HAPP_ID = "ec-uid-cap-test"
...
describe("Client-Server Scenarios", () => {
  ...
  it.only("should make valid anonymous zome call", async function() {
    ...
    try {
      const { responseOne } = await page.evaluate(async function (host_agent_id, ec_happ_id) {
        ...
        const client = new Chaperone({
          ...
          app_id: ec_happ_id, // NOT RANDOM: this needs to match the hash of app in hha
        ...
        if (client.anonymous === false) {
          throw new Error("Client is signed-in")
        }

        let responseOne
        try {
          responseOne = await client.callZomeFunction(`ec-cap-v1`, "chat", "agent_stats", null);
        } catch (err) {
          console.log(typeof err.stack, err.stack.toString());
          throw err
        }
  ...
  it('Should recover from internet loss (client closing) in the middle of a zome call', async function () {
    ...
    try {
      ...
      const { hasSignedUp, areEnvoyConnectionsValid, isChaperoneValid, responseSuccess } = await page.evaluate(async function (host_agent_id, registered_happ_hash, joiningCode) {
        ...
        if (client.conn.socket.socket.readyState !== 1) {
          await client.conn.opened(200_000)
        }
        await client.ready(200_000)
