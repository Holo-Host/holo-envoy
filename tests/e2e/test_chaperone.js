...
const EC_HAPP_ID = "uhCkklzn8qJaPj2t-sbQmGLdEMaaRHtr_cCqWsmP6nlboU4dD"
  ...
  it.only("should make valid anonymous zome call", async function() {
    ...
    try {
      const { anonymousCallResponse } = await page.evaluate(async function (host_agent_id, ec_happ_id) {
        ...
        let anonymousCallResponse
        try {
          anonymousCallResponse = await client.callZomeFunction('elemental-chat', "chat", "agent_stats", null);
        } catch (err) {
          console.log(typeof err.stack, err.stack.toString());
          throw err
        }
      ...
      expect(anonymousCallResponse).to.have.property("active").which.equals(0)
    } finally {
    }
  })
